...
class ResponseError {
  ...
  constructor(raw: { code: string; message: string; data: any }) {
    this.code = raw.code;
  ...
  toError(): Error {
    ...
    return new TypeError(
      `${this.code}: ${this.message} (${JSON.stringify(this.data)})`
    );
  }
}
...
class ManyErrors extends Error {
  ...
  constructor(errors: Array<Error>) {
    super(
      `${errors.length} errors: ${errors.map((e) => e.toString()).join(", ")}`
    );
...
export class Handler<R, V> {
  ...
  private cSet(ref: R, v: V) {
    window.localStorage.setItem(
      this.key(ref),
      JSON.stringify({ time: new Date(), v })
    );
  }
  ...
  async total(): Promise<number> {
    const r = await this.client._fetch<{ total: number }>(
      `generic/total/${this.name}`,
      {
        method: "GET",
      }
    );
...
class Client extends BaseClient {
  ...
  async deallocTaf(tafid: UUID): Promise<void> {
    const r = await this.fetch<void>(
      `config/ax/taf/dealloc?tafid=${encodeURIComponent(tafid)}`,
      {
        method: "POST",
  ...
  async verifyTaf(
    ...
    const r = await this.fetch<{ done: boolean; feedback? }>(
  ...
  async stepAzrq(azrqid: UUID): Promise<Grant | null> {
    ...
    args.azrqid = azrqid;
    const r = await this.fetch<{ grant: Grant }>(
      `oauth/az/step?${new URLSearchParams(args)}`,
      {
        method: "POST",
      }
    );
  ...
  async oclientsList(): Promise<Array<OClient2>> {
    const r = await this.fetch<{ oclients: Array<OClient2> }>(
      "oauth/oclients",
      {
        method: "GET",
      }
  ...
  async emailInfo(token: string): Promise<string> {
    const r = await this.fetch<{ email: string }>(
      `email/info?token=${encodeURIComponent(token)}`,
      {
        method: "GET",
      }
    );
