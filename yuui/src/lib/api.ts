...
class Client {
	...
	async username(slug: string): Promise<{ exists: boolean }> {
		const r = await fetch(
			new URL(`/api/v1/user/exists?slug=${encodeURIComponent(slug)}`, this.baseUrl.href).href,
			{
				method: 'GET'
			}
		)
	...
	async loginAttempt(
		afid: string,
		attempt: string
	): Promise<{ success: false; msg: string } | { success: true; done: boolean }> {
	...
	async getAzrq(azrqid: UUID): Promise<Grant | null> {
		const r = await fetch(
			new URL(`/api/v1/oauth/azrq?azrqid=${encodeURIComponent(azrqid)}`, this.baseUrl.href).href,
			{
				...(await this.commonOpts('GET'))
			}
		)
